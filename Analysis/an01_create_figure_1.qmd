...
abstract: |
  ...
  Figure 1 shows a map of the median RACMO melt in Antarctica (1980--2018).
  ...
  shown on the figure, using the AMSR melt dataset of Picard (2022). 
  The correlation is performed over the contemporary periods (2003--2018).
  
  !! To calculate & plot the correlations, it is necessary to have first run 
  dt03, which prepares the AMSR data.
---
...
meltKulas <- kulaR::kulaS(count = 18, addLast = "black")
meltRange <- c(0, 285)
```
...
fileName <- paste0(figPath, "fig01v", sprintf("%02d", vv),
                   "_medianMelt_map-an01.pdf")
...
terra::plot(x = racmoMedian,
            ...
            mar = c(0.55, 0, 0.55, 0),
            xlim = c(-26, 26), ylim = c(-19, 21.5))

# Add stipling for insignificant AMSR-RACMO correlations (i.e. p > 0.05)
terra::points(insig5, pch = 4, col = kulaL(3), cex = 0.01)
