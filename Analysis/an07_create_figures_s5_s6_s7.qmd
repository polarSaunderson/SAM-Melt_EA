...
abstract: |
  ...
  For Figure S5:
    figName:   "S5"
    racmoVars: ["swsd", "t2m", "lwsd"]
    plotType:  ["mean", "mean", "mean"]
...
for (ii in 1:varCount) {
  ...
  if (iiType %in% c("mean", "stdev")) {
    iiData <- paste0(klimaPath, iiVar, "-dt07.json") |>
      domR::read_list() |>
      _[[iiWinType]]$regionalMeans
    if (iiType == "mean") iiData <- iiData$klimaMean
    if (iiType == "stdev") iiData <- iiData$klimaSD
  } else if (iiType == "koral") {
    iiData <- paste0(koralPath, iiVar, "-dt07.json") |>
      domR::read_list() |>
      _$koralCubud[[iiWinType]]$regionalMeans
  } else {
...
It works here, but it is very unconstrained - the results can easily be wrong.
```{r}
# Preallocate
nameList <- list()
