...
abstract: |
  ...
  The regressions are run in this notebook (>2-5 minutes) and saved as NetCDFs.
  They can then be plotted in an03.
  
  The NetCDF output has three layers:
  ...
  The following ERA5 variables can also be used; these are slow! (> 25 min)
    - MSLP, z700
  ...
  It is possible to run the regressions for a single month (e.g. the regression 
  of the Dec variable onto the Dec SAM), or using a multi-month average (e.g. 
  the mean (e.g. t2m) or total (e.g. snowmelt) Dec-Jan variable onto the mean 
  Dec-Jan SAM).
  ...
  !! There is a conversion from Joules to Watts which assumes that all of the 
     ...
     chunk (line 120). Any changes to account for non-31 day months will need to
     be hard-coded in.
...
variables it makes sense to think of the mean value (i.e. the average of the Dec
and Jan t2m), whereas others make more sense as totals (i.e. the sum of the Dec
and Jan snowmelt).
```{r}
...
regression <- terra::app(x = varAustral,   # Calc the change in this variable
                         ...
                         fun = apply_lm)   # Local function in the R/ directory
...
Makes it easier to plot in a loop in an03.
...
blurb <- paste("Regression of", params$regressVar, "onto the", params$samIndex, 
               ...
               "of the Dec and Jan snowmelt). The SAM data is the mean of the",
               "monthly values each summer (e.g. average of the Dec and Jan",
               "SAM value).")
```
