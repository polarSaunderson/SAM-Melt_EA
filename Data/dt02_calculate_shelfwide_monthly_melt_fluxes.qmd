---
title: dt02_calculate_shelfwide_monthly_melt_fluxes
abstract: |
  The purpose of this notebook is to calculate a shelfwide average melt flux 
  each month from the RACMO data. 
  We use the RACMO2.3p3 monthly melt data from van Dalum et al. (2022), which 
  provides melt values as kg m-2 (i.e. the total mass of meltwater, in kg, that 
  is produced in each 1x1m square of the pixel over the full month). 
  We calculate this for 27 shelves in East Antarctica (see dt00), and only 
  include pixels that are at least 50% ice shelf (based on MEaSURES v2; Mouginot 
  et al. 2017).
  Values are extracted for each pixel in a shelf that meets this criteria, then 
  summed and averaged by the total extracted area. 
  The final units are kg m-2 (per month).
params:
  months:  [11, 12, 1, 2]
...
## Preallocate
```{r}
meltList   <- list()      # holds a matrix of melt values for each month
...
## Metadata
```{r}
# Add metadata to the meltList itself
blurb <- paste("These values are the average melt produced (in kg) for each",
               ...
               "RACMO pixels which are at least 50% ice shelf are included. The",
               "data is from RACMO2.3p3 (van Dalum et al., 2021).")
